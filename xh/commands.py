...
def insert_command(database: DBInterface, command: str, timestamp: int) -> None:
...
def get_all_unique_commands(database: DBInterface) -> list[str]:
    """Collect all unique Commands from Database."""
    commands: list[str] = database.get_all_unique_commands()
    return commands


def get_top_commands(database: DBInterface, number: int) -> str:
